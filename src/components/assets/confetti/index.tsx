...
// This was slightly refactored to use with TypeScript,
// so check the original component in https://github.com/ektogamat/r3f-confetti-component
...
export const ExplosionConfetti = ({
  ...
  const [booms, setBooms] = useState<any[]>([])

  rate = rate / 100
  const geometry = new THREE.PlaneGeometry(0.03, 0.03, 1, 1)

  function explode() {
    const boom = new THREE.Object3D() as any
    ...
    if (ref.current) ref.current.add(boom)
    booms.push(boom)
